...
>
	...
	<div class="flex items-center gap-2 border-b border-surface-border bg-surface-light px-3 py-1.5">
		<div class="flex gap-1.5">
			...
			<span class="h-2 w-2 rounded-full bg-matrix/70"></span>
		</div>
		<span class="text-[10px] tracking-wider text-text-dim">SENTINEL TERMINAL v2.0.4</span>
	</div>
	...
	<div class="max-h-64 space-y-0.5 overflow-y-auto p-3">
		...
		<div class="flex gap-2">
			<span class="text-matrix-dim">{prompt}</span>
			<span class="inline-block w-2 bg-matrix" style="animation: typing-cursor 1s step-end infinite;"
				>&nbsp;</span
			>
		</div>
	</div>
